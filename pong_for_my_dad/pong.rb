...
$song = Music.new('my plea.wav'); $song.loop = true; $song.play; $song.volume = 20
$ballDirection = :none; $start = false; $rmed = 3; $rmedB = 3
$scr1 = 0; $scr2 = 0; $startB = false; $mstopped = false; $bspeed = 2
$diff = :impossible; $paused = true; $rmedBb = 3; $frames = 0
...
def startGame(tmp = false, oth = false); clear; $b = Square.new(color: "white", size: 25, x: 475, y: 277)
  if tmp; $p1.y = 250; $right.y = 250; $scr1 = 0; $scr2 = 0; $frames = 0; end
...
on :key_down do |e|; abort if e.key == "escape"
if e.key == "m"; case $mstopped; when true; $mstopped = 3
$song.pause; when false; $mstopped = true; $song.resume; end; end
$mstopped = false if $mstopped == 3; end; on :key_held do |e|
  $p1.y -= 3 if (e.key == "up") && $p1.y > 0 && $startB && !$paused
  $p1.y += 3 if (e.key == "down") && $p1.y < 500 && $startB && !$paused
 if e.key == "return" && !$startB; $start = true; $frames = 0; $p1.y = 250
  $right.y = 250; $scr1 = 0; $scr2 = 0; $frames = 0; $rmed = 3; $rmedB = 3
  $p1.y = 250; $right.y = 250; end; case e.key; when "1"
...
Text.new("Press 3 for Impossible Mode", y: 460, x: 360, size: 25)
Text.new("Press M to toggle music.", y: 480, x: 360, size: 25); update do; if $startB == false
    ...
    unless $paused == true; case $ballDirection; when :leftdown
      $b.x -= $bspeed; $b.y += $bspeed; when :leftup; $b.x -= $bspeed
      $b.y -= $bspeed; when :rightdown; $b.x += $bspeed; $b.y += $bspeed
    when :rightup; $b.x += $bspeed; $b.y -= $bspeed; end; end
    if $b.y > 575; $b.y -= $bspeed; $ballDirection = wallHit;
    elsif $b.y < 0; $b.y += $bspeed; $ballDirection = wallHit; end
    counter = $p1top; until counter == $p1low
    if $b.contains?(60, counter); $b.x += $bspeed; $ballDirection = wallHit(true)
    end; counter += 1; end; counter = $cputop; until counter == $cpulow
    if $b.contains?(940, counter); $b.x -= $bspeed; $ballDirection = wallHit(true);$rmed=3;$rmedB=3;
    ...
    if $diff == :medium; $rmed = rand(0..2) if $rmed == 3; if $rmed == 1 || 2
    ...
    else; $rmedB = 0; end; end; end; end; if $diff == :easy
    $rmedBb = rand(0..1) if $rmedBb == 3
    case $rmedBb; when 0; if $right.y < 505; $right.y += 3
    else; $rmedBb = 1; end; when 1; if $right.y > 0; $right.y -= 3
  else; $rmedBb = 0; end; end; end
if $startB && !$paused; $divf = $frames / 240; $bspeed = 2
  for nn in 1..$divf do; $bspeed += 0.12; end; end
    ...
    $right.y = 0 if $right.y < 0; end; $song.loop = true; $frames += 1; end; show
